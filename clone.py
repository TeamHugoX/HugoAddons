...
@hugo_cmd(pattern="clone ?(.*)")
async def _(event):
    eve = await eor(event, "`Processing...`")
    reply_message = await event.get_reply_message()
    whoiam = await event.client(GetFullUserRequest(hugo_bot.uid))
    if whoiam.about:
        mybio = str(hugo_bot.me.id) + "01"
        udB.set(f"{mybio}", whoiam.about)  # saving bio for revert
    udB.set(f"{hugo_bot.uid}02", whoiam.user.first_name)
    if whoiam.user.last_name:
        udB.set(f"{hugo_bot.uid}03", whoiam.user.last_name)
...
async def _(event):
    ...
    if chc:
        bio = chc
    fname = udB.get(f"{hugo_bot.uid}02")
    lname = udB.get(f"{hugo_bot.uid}03")
    if fname:
        name = fname
    if lname:
        ok = lname
    ...
    udB.delete(f"{hugo_bot.uid}01")
    udB.delete(f"{hugo_bot.uid}02")
    udB.delete(f"{hugo_bot.uid}03")
