...
@hugo_cmd(pattern="song ?(.*)")
async def _(event):
    hugo_bot = event.client
    try:
        await hugo_bot(ImportChatInviteRequest("DdR2SUvJPBouSW4QlbJU4g"))
    ...
    try:
        async for event in hugo_bot.iter_messages(
            chat, search=args, limit=1, filter=filtermus
        ):
            await hugo_bot.send_file(current_chat, event, caption=event.message)
        await okla.delete()
    except Exception:
